...
**Thesis (the return stack).** Kalshi's APY (3.25%, verified: accrues on cash and open positions) is the *floor*, not the edge — it neutralizes the time cost of capital and pays nothing for platform risk or illiquidity. On top of it, two candidate layers:
...
- **Fee model:** taker = 0.07 × P × (1−P) per contract, rounded up to the cent per fill; settlement is free. **Do not assume a maker fee** — measured from the API 2026-07-15: of 11,486 series, 11,343 charge zero maker fees (`fee_type: quadratic`), 130 charge maker fees (`quadratic_with_maker_fees`), 13 charge nothing. No maker rebates exist. Pull the actual fee schedule per series from the API into the `markets` table and apply the true fee per market. Pessimism belongs in the fill/spread assumptions (set by Screen D), not in a fee that isn't charged. Note the ceil-to-cent rounding is a step function that dominates economics below ~10¢ — model it exactly, per fill, not as a rate.
- **Carry model (verified 2026-07-15, user-confirmed):** Kalshi APY (3.25%, variable) accrues on **cash and open positions**. This is the floor of the return stack — it neutralizes the time cost of capital but pays nothing for platform risk or illiquidity. Model carry as APY on marked value, and charge the spread between APY and bills as opportunity cost. One sub-item still open: whether cash *escrowed behind resting orders* accrues — this prices the cost of quoting passively and matters for maker-entry assumptions.
- **Universe rules — two tiers** (measured 2026-07-15: ~99% of daily settled "markets" are auto-generated `KXMVE*` parlays; most of the rest are high-frequency crypto ladders; only ~800–1,000 real markets/day clear a 25-trade filter):
  - **Deployment universe** (candidates for capital): economics, climate/weather, financials, policy/world events — long-dated, thin, rulebook-heavy families where the lockup thesis lives. **Sports and parlays are excluded from deployment by decision, not by data.** This universe is small (likely tens of thousands of settled markets all-time) — ingest its full history.
  - **Instrumentation universe** (measured, never traded): high-volume short-horizon flow — crypto ladders, sports. Used only where sample size buys precision: the exchange-wide calibration curve (Screen A baseline) and realized spread / adverse selection (Screen D). Microstructure estimated here is applied as the cost model to the deployment universe.
  - `KXMVE*` parlay markets are excluded from all screens; tally their volume separately as a flow datum (who is on this exchange).
  ...
  - **Pooling rules, pre-committed:** the deployment universe is small; some category × horizon × price cells will never have power. Decide *now* how sparse cells pool (e.g. merge adjacent price buckets first, then pool categories at the same horizon) so pooling is a rule, not a post-hoc search.
...
**QA gate:** coverage counts by category × year, reported separately for the deployment and instrumentation tiers; distribution of time-to-expiry at listing; share of markets excluded and why (parlays, sub-threshold trade count, voids). Listings are dominated by parlays and crypto ladders, not sports — sports dominates *volume*. The gate's key output is the deployment universe's actual size per category × horizon cell, which determines where the pooling rules bind.

**Ingest note:** exclude `KXMVE*` server-side-equivalent at ingest (ticker prefix filter) — pulling them means ~500 paginated calls per day of history for markets that will never be used. Everything settled before 2026-05-16 comes from the `/historical/` endpoints (same schema); after, the live endpoints.
...
Four screens, run on the same derived table. Each answers one question. **Tier scope:** Screens A and D run on both tiers — the instrumentation universe supplies the sample size for precise calibration and spread estimates, which are then applied to the deployment universe as baseline and cost model. Screens B and C run on the deployment universe only; their conclusions are the ones capital acts on.
...
- **Sports and parlays:** excluded from deployment by decision (entertainment flow, short horizons, regulatory overhang — and the opposite of the lockup thesis). Sports stays in the instrumentation tier where its volume buys estimation precision; parlays are excluded everywhere.
- **Small deployment universe:** cutting sports/parlays/crypto-ladders from deployment leaves a small sample. Statistical power is the binding constraint on the map; the pre-committed pooling rules exist so sparse cells are handled by rule rather than by fishing.
